...
function getSubscriptionsDay(){
	...
	var sql = "SELECT COUNT(*) FROM subscription INNER JOIN project ON subscription.projectID = project.projectID WHERE DATE(subscription.date_added) = '"+ date +"' AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
...
function getDonationsNumberDay(){
	var orgNr = localStorage.getItem('orgNr');
	var date = $("input[name=dateStats]").getItem(); //dateInput
	var sql = "SELECT COUNT(*) FROM donation INNER JOIN project ON donation.projectID = project.projectID WHERE DATE(donation.date) = '"+ date +"' AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
...
function getDonationsSumDay(){
	var orgNr = localStorage.getItem('orgNr');
	var date = $("input[name=dateStats]").getItem(); //dateInput
	var sql = "SELECT SUM(sum) FROM donation INNER JOIN project ON donation.projectID = project.projectID WHERE DATE(donation.date) = '"+ date +"' AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
...
function getDonationsAverageDay(){
	var orgNr = localStorage.getItem('orgNr');
	var sql = "SELECT COUNT(*) FROM subscription INNER JOIN project ON subscription.projectID = project.projectID WHERE DATE(donation.date) = '" + date + "' AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
	...
	$.ajax({
		...
		success : function(json){
			var res = parseInt(json);
			var sql2 = "SELECT SUM(sum) FROM subscription INNER JOIN project ON subscription.projectID = project.projectID WHERE DATE(donation.date) = '" + date + "' AND project.organizationNr = '"+ orgNr +"'";
			var data2 = {"getSQL" : sql2};
			$.ajax({
				type : "POST",
				url : url,
...
function getNewsNumberDay(){
	var orgNr = localStorage.getItem('orgNr');
	var date = $("input[name=dateStats]").getItem(); //dateInput
	var sql = "SELECT COUNT(*) FROM news INNER JOIN project ON news.projectID = project.projectID WHERE DATE(news.date_added) = '"+ date +"' AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
...
function getSubscriptionsMonth(){
	var orgNr = localStorage.getItem('orgNr');
	var date = $("input[name=dateStats]").getItem(); //dateInput
	var sql = "SELECT COUNT(*) FROM subscription INNER JOIN project ON subscription.projectID = project.projectID WHERE MONTH(date(subscription.date_added)) = MONTH('" + date + "') AND YEAR(date(subscription.date_added)) = YEAR('" + date + "') AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
...
function getDonationsNumberMonth(){
	var orgNr = localStorage.getItem('orgNr');
	var date = $("input[name=dateStats]").getItem(); //dateInput
	var sql = "SELECT COUNT(*) FROM donation INNER JOIN project ON donation.projectID = project.projectID WHERE MONTH(date(donation.date)) = MONTH('" + date + "') AND YEAR(date(donation.date)) = YEAR('" + date + "') AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
...
function getDonationsSumMonth(){
	var orgNr = localStorage.getItem('orgNr');
	var date = $("input[name=dateStats]").getItem(); //dateInput
	var sql = "SELECT SUM(sum) FROM donation INNER JOIN project ON donation.projectID = project.projectID WHERE MONTH(date(donation.date)) = MONTH('" + date + "') AND YEAR(date(donation.date)) = YEAR('" + date + "') AND project.organizationNr = '"+ orgNr +"'";
	var url = getURLappBackend();
	var data = {"getSQL" : sql};
